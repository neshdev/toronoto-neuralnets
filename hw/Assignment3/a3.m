function [data_loss, data_names, class_loss, class_perf] = a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  data_loss = zeros(1,3)
  class_loss = zeros(1,3)
  class_perf = zeros(1,3)
  
  for data_i = 1:3,
    data = datas2{data_i};
    data_name = data_names{data_i};
    loss_i = loss(model, data, wd_coefficient);
...
function [loss, grad] = cross_entropy_loss(x,y)
   N = size(x)(2);
   probs = softmax_custom(x);
   
   temp = probs .* y;
   [r,c] = find(temp);
   idx = sub2ind(size(y), r,c);
   loss_row = temp(idx);
   
   loss = sum(-log(loss_row));
   #loss = sum(-log(probs .* y))/N;
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  dW2 = dW2 + wd_coefficient * model.hid_to_class;
  dW1 = dW1 + wd_coefficient * model.input_to_hid;
